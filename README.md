...
graph TB
    ...
    subgraph "Система сбора и прогноза"
        direction LR
    ...
    %% Поток 1: Сбор данных (Асинхронный, сплошная линия)
    WS -- "POST /data" --> GW
    GW -- "Публикация" --> RMQ
    RMQ -- "Обработка" --> PS
    PS -- "Сохранение" --> DB

    %% Поток 2: Запрос прогноза (Синхронный, пунктирная линия)
    Client -.-> |"GET /forecast"| GW
    GW -.-> |"HTTP-запрос"| PS
    PS -.-> |"Чтение<br/>истории"| DB
